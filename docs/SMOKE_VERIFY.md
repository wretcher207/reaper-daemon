...
2. Make sure `bridge/bridge_config.json` has `"allow_audio_writes": true`
   (`python3 setup/install.py --allow-audio-writes` sets it; the older
   `allow_risk_level_3: true` still works as a fallback for every gate).
