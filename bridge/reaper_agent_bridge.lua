...
-- automation.write_command in automation.lua): the transactional engine it
-- shares with apply_automation_transaction is defined after the table.

-- FX-envelope automation lives in automation.lua, next to this file. It is
-- found the way require finds json but loads fresh on every bridge load, never
-- from require's cache, so reload_bridge picks up an edited copy. Keeping it
-- separate holds this file under the 256 KiB the Claude plugin directory
-- inspects in full.
local automation = assert(loadfile(assert(package.searchpath("automation", package.path))))({
  finite_or_nil = finite_or_nil,
  bar_from_time = bar_from_time,
  find_fx = find_fx,
  find_fx_param = find_fx_param,
  time_from_point = time_from_point,
  envelope_shape = envelope_shape,
  resolve_position = resolve_position,
  fx_summary = fx_summary,
  AUTOMATION_MODE_NAMES = AUTOMATION_MODE_NAMES,
})
